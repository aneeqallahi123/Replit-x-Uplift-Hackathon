Make the following changes to this project, then commit and push them to
the connected GitHub repository.

## 1. Create a new file: `dastak-clone/assets/js/maryam-agent.js`
...
## 2. Edit three existing HTML files

In each of these three files:
- `dastak-clone/index.html`
- `dastak-clone/services.html`
- `dastak-clone/apply.html`

Find the closing `</body>` tag and insert these two lines immediately
before it (after all existing script tags already in the file):
...
Do not remove or modify any existing script tags, styles, or markup in
these files — only add these two lines right before `</body>`.

## 3. What this accomplishes (context, not action)

This connects the site to a voice AI assistant ("Maryam", built on Uplift
AI + LiveKit) that can:
- Speak with the user in Urdu and guide them through renewing a driving
  license
- Show a pulsing red dot on specific buttons/cards to tell the user what
  to click next, and wait for the real click before continuing
- Fill in the 5 application form fields (CNIC, License Number, License
  Issuance Date, Renewal Duration, Is Old License in Possession)
  directly once the user confirms each value by voice
- Keep the same voice conversation alive across page navigations
  (homepage → services → application form) using sessionStorage, since
  each of those is a full page reload in this site

No existing design, styling, or click-handling logic should be changed —
this only adds new behavior alongside what's already there.

## 4. After making these changes

1. Verify the three HTML files still open and render normally
2. Stage all changes (the new `maryam-agent.js` file and the three
   edited HTML files)
3. Commit with the message: "Add Maryam voice agent integration"
4. Push the commit to the connected GitHub repository's default branch
